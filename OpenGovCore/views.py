...
class Home(View):
    ...
    def get(self, request):
        latest_bills = Bills.objects.filter(
            status='Passed').order_by('-date_of_introduction')[:5]
        latest_ques = Questions.objects.all().order_by('-date')[:5]
        latest_deb = Debates.objects.all().order_by('-date')[:5]
        latest_lap_bills = Bills.objects.filter(
            status='Lapsed')[:5]  # .order_by('-date')[:5]
...
class MembersBySession(View):
    ...
    def get(self, request, session):
        ...
        cand_data = []
        for i in attendance:
            candidate_data = Candidature.objects.filter(
...
class MembersByHouse(View):
    ...
    def get(self, request, house):
        ...
        data = Candidature.objects.filter(
            central_legislature_id=centrail_leg_id)
...
class MembersByParty(View):
    ...
    def get(self, request, house, party):
        ...
        centrail_leg_id = Central_Legislatures.objects.get(name=house)
        data = Candidature.objects.filter(
            party_id=party_obj, central_legislature_id=centrail_leg_id)
        page = request.GET.get('page', 1)
        paginator = Paginator(data, 10)
        try:
            data = paginator.page(page)
...
class MembersByState(View):
    ...
    def get(self, request, house, state):
        ...
        centrail_leg_id = Central_Legislatures.objects.get(name=house)
        data = Candidature.objects.filter(
            state_id=state_obj, central_legislature_id=centrail_leg_id)
        page = request.GET.get('page', 1)
        paginator = Paginator(data, 10)
        try:
            data = paginator.page(page)
...
class MembersByConstituency(View):
    ...
    def get(self, request, house, constituency):
        ...
        constit_obj = Parliamentary_Constituencies.objects.filter(
            name=constituency)
        centrail_leg_id = Central_Legislatures.objects.get(name=house)
        data = []
        for i in constit_obj:
            data1 = Candidature.objects.filter(
                parliamentary_constituency_id=i, central_legislature_id=centrail_leg_id)
            for j in data1:
                data.append(j)
...
class MemberInfo(View):
    ...
    def get(self, request, house, name):
        client = gnewsclient.NewsClient(
            language='english', location='india', topic=name, max_results=3)
        news = {}
        news = (client.get_news())
        def getValues(key, inputData): return [
            subVal[key] for subVal in inputData if key in subVal]
        g_news_title = getValues('title', news)
        g_news_links = getValues('link', news)

        candidate_obj = Candidate.objects.get(name=name)
        centrail_leg_id = Central_Legislatures.objects.get(name=house)
        candidature_obj = Candidature.objects.filter(
